...
export default async function download(
  ...
  title: string,
  thumbnail: string,
  isCropRequired: boolean,
  targetVolume: number
) {
  const [[{ originalVolume, newVolume }, { moodValue }], { meanHue }] = await Promise.all([
    (async () => {
      await downloadMusic(youtubeLink, artist, title);
      return await Promise.all([normalizeVolume(artist, title, targetVolume), extractMoodValue(artist, title)]);
    })(),
    downloadAndCropThumbnail(thumbnail, artist, title, isCropRequired),
  ]);

  await saveMetadata(artist, title, { meanHue, moodValue, originalVolume, newVolume });
}
...
async function normalizeVolume(artist: string, title: string, targetVolume: number) {
  ...
  const command = `ffmpeg -y -i "${inputPath}" -filter:a "volume=${targetVolume}dB" "${outputPath}"`;
  try {
    // 먼저 원래 볼륨을 측정합니다.
