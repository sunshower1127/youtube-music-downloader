...
export async function getAverageColor(thumbnailURL: string): Promise<string> {
  // URL에서 이미지를 받아오기
  const response = await fetch(thumbnailURL);
  if (!response.ok) {
    throw new Error(`이미지 불러오기 실패: ${response.statusText}`);
  ...
  // 이미지를 16x16 크기로 축소
  const resizedImage = await image.resize(1, 1).raw().toBuffer({ resolveWithObject: true });
  const [r, g, b] = resizedImage.data;
